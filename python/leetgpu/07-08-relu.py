...
@dsl_user_op
def relu(x, *, loc=None, ip=None):
    zeros = cute.zeros_like(x)
    return cute.where(x > 0, x, zeros, loc=loc, ip=ip)
...
def relu_kernel(
    ...
    output_frag_thr.store(relu(input_frag_thr.load()))
...
def solve(input: cute.Tensor, output: cute.Tensor, N: cute.Int32):
    ...
    relu_kernel(input, output, tiled_copy, N).launch(grid=grid_size, block=block_size)
...
def test():
    ...
    torch.testing.assert_close(output, torch.relu(input))
    print("Test passed!")
